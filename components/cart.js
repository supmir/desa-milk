...
export default function Cart(props){
    ...
    function cartQty(){
        ...
        0 : Object.entries(cart).reduce((total, [key, {qty}])=>total+qty, 0)
    }

    function cartTotal(){
        return cartQty()===0?0: Object.entries(cart).reduce((total, [key, {qty,price}])=>total+qty*price, 0)
    }
    function discountQty(){
        return discounts && 
        Object.keys(discounts).length === 0 &&
        Object.getPrototypeOf(discounts) === Object.prototype ? 
        0 : Object.entries(discounts).reduce((total, [key, {}])=>total+1, 0)
    }

    function discountTotal(){
        return discountQty()===0?0: Object.entries(discounts).reduce((total, [key, {price}])=>total+price, 0)
    }
    ...
    <tbody>
      ...
      ))}
      {
        cartQty()<6 &&cartQty()!==0 &&
      <tr>
        <td colSpan="3" className="text-center">Add {6 - cartQty()} more items to get a discount</td>
      </tr>
      }
    </tbody>
      {
